...
# Paste your raw PlantUML node definitions here
plantuml_nodes = """
...
...
"""  # (truncate for now or replace with full set)

# Regex pattern to extract full node line and its level number (even decimals)
pattern = r'^(.*?Level\s+(\d+(?:\.\d+)?)[^"]*".*?)$'
matches = re.findall(pattern, plantuml_nodes, flags=re.MULTILINE)

# Sort nodes numerically by level number
def parse_level_num(s):
    try:
        return float(s)
    except:
        return float('inf')

sorted_lines = sorted(matches, key=lambda m: parse_level_num(m[1]))
